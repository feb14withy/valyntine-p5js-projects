...
function draw() {
  ...
  //check every frame how many frames it's been since we clicked.
  var framesSinceClick = frameCount - clickFrame;

  //if it's been longer than the clicked frame, change the angle. extra argument is to check if we've clicked (clickFrame == -1 before the sketch starts)
  if(framesSinceClick < waveFrames && clickFrame > 0)
  {
    waveDegrees = WAVE_OFFSET_ANGLE;
  }
  else if (framesSinceClick > waveFrames)
  {
    waveDegrees = 0;
  }

  print(framesSinceClick, waveFrames);
}
